...
usage() {
  cat <<'EOF'
Run OWASP Dependency-Check on OSS and NES Spring PetClinic branches.
...
Optional (defaults shown):
  ...
  --out         Where to write reports (default: <script-dir>/dependency-check-output)

Example:
  ./run.sh

Install OWASP CLI:
  Check the link https://owasp.org/www-project-dependency-check/
EOF
}
...
WORKDIR=""
OUT_DIR="$SCRIPT_DIR/dependency-check-output"
...
if ! command -v dependency-check >/dev/null 2>&1 && ! command -v dependency-check.sh >/dev/null 2>&1; then
  echo "dependency-check CLI not found on PATH." >&2
  exit 1
fi

DC_BIN="$(command -v dependency-check || command -v dependency-check.sh)"
...
scan_dir() {
  local label="$1" dir="$2"
  local outfile="$OUT_DIR/${label}-petclinic-dependency-check.json"
  log "Scanning $dir -> $outfile"
  "$DC_BIN" \
    --project "${label^^} PetClinic" \
    --format JSON \
    --out "$outfile" \
    --scan "$dir" \
    --failOnCVSS 0
